...
Score each row *held* / *broke* / *n-a (no data)*, with a cited instance (transcript line, commit, PR, or check result). "Broke" is not failure — it is the data the loop exists to surface. An honest *n-a* beats a guessed pass.
...
- **Loop changes** (→ `design-loop.md`): *…each tied to its scorecard evidence.*
- **Harness changes** (→ skill / lint / check / checklist / template): *…each tied to its evidence.*
