...
**2 — Size.** Triage by blast-radius and reversibility. Irreversible or cross-cutting → the full note. Local and reversible → skip to build. Most changes are the latter; spend the ceremony where undoing is expensive. **An option may not be eliminated on an estimated cost** — agent effort estimates run low in the direction that matters, so if cost is the ground for ruling something out, measure it (a spike in a throwaway worktree) before it decides anything. Eliminating on a *stance* is legitimate; eliminating on a guess is not.
...
**4 — De-risk.** Identify the assumption the design rests on and *test it* — eval it, prototype it, read the pinned source — recording the result in the note's De-risk evidence section. State what stays unverified rather than implying coverage you do not have — and apply that inventory **symmetrically**: verification effort naturally follows the chosen route, so the claims that *eliminated* the alternatives are the ones that go unchecked (§De-risk evidence, the #763 scorecard). A note is a proposal, not a guarantee. **The abandon exit is first-class:** a de-risk result that kills the design is a real outcome, not a failure — record the negative result in the design note itself, close the driving issue with it, and stop the loop there.
...
- **Forks are numbered questions with a stated lean.** One subject per turn; give a recommendation and why; record each ruling in the note. A parked subtopic is named and honoured, never silently dropped. **A ruling that eliminates an option on a factual claim about the outside world carries that claim's verification status** — checked at source and dated, or flagged unchecked — *before* the ruling is recorded. A claim is load-bearing whenever it eliminates an option, and elimination usually happens here, at the ruling, not later at de-risk. Inside these stages the propose-order stance inverts by design (workflow.md §"Propose order; don't multi-question"): eliciting the ruling *is* the work, not stalled momentum.
