...
| Doc                                        | Subject                                                                                                                                                                                                                                                   | Landed         |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| [keybinds.md](./keybinds.md)               | Cross-platform keybind taxonomy (audited target) — `Hyper` command layer + `Super` (Cmd-parity); per-platform realization via keyd / Karabiner / niri / skhd + the registry                                                                               | #80            |
| [fonts.md](./fonts.md)                     | Font selections + the fontconfig-conducted runtime model                                                                                                                                                                                                  | #81 + #82      |
...
| [screen-capture.md](./screen-capture.md)   | Non-interactive screen capture for remote/agent visual verification — `grim` (wlr-screencopy) over SSH; niri's interactive screenshot UI stays the console path; `block-out-from` for credential surfaces                                                 | #529           |
...
| [bluetooth.md](./bluetooth.md)             | Bluetooth peripherals — BlueZ via desktop-env (#773; ex-mobility #636); pairing UI rides Noctalia, pairings persist across the ephemeral root; sharp edges: upstream pair-without-connect (noctalia#3867), stale-session panel after switch               | #773 + #781    |
...
| [file-manager.md](./file-manager.md)       | Nautilus + gvfs — GUI complement to yazi (drag-and-drop, thumbnails, MTP/SMB/`trash://`); shared freedesktop trash with declarative 30-day purge; fires removable-media.md's revisit clause; swapped from Thunar for runtime theme/polarity compatibility | #762 + #771    |
...
| [utm.md](./utm.md)                                         | **Retired from the fleet (#726)** — `pkgs.utm` dropped from both Macs; UTM had hosted nixos-vm (decommissioned #634) on celaeno and the operator's own VMs on saturn. Retained as the selection record                                                                                                     | #11    |
...
| [hammerspoon.md](./hammerspoon.md)                         | **Decommissioned (ADR-040, #494)** — retired from the macOS interaction stack; yabai + skhd + Karabiner only (ADR-047). Retained as the selection record for the Hammerspoon era                                                                                                                           | #11    |
| [macos-window-management.md](./macos-window-management.md) | **Superseded (ADR-040, #494)** — the pure-Hammerspoon / native-fullscreen-Spaces model is replaced by a tiling WM. Retained as the selection record; current decision → [ADR-047](../decisions/ADR-047-macos-window-manager-yabai.md) (the design note this row used to point to is itself now superseded) | #440   |
...
**Deliberate no-doc:** #77 (Cursor IDE) landed without a per-tool selection doc — Cursor is a foregone install, not a selection weighed against alternatives. This is the documented exception to the §"Doc precedes implementation" rule in [workflow.md](../workflow.md): it applies to *selections*, not to foregone installs where no alternative was weighed.

That rationale originally lived in the head comments of `home/nixos/cursor-ide.nix`; it moved into [`cursor.md`](./cursor.md) when Cursor was removed from the Linux desktop and Darwin became its only install platform. So `cursor.md` now carries two layers: the foregone-install rationale (no alternatives weighed) and the ADR-031 install-path selection (cask vs. `pkgs.code-cursor`), which *is* a selection-with-alternatives. The "no-doc" precedent for IDE-vs-IDE remains intact — the IDE-selection question was never litigated, only relocated.
