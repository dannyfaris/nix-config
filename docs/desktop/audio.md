...
**Theming needs no new wiring.** The waybar module rides the existing `stylix.targets.waybar`; `pwvucontrol` rides Noctalia's own gtk3/gtk4 builtin templates for both polarity (the template's `apply.sh` `gsettings` write, ADR-048) and accent/background colors. No new Stylix target is added.
