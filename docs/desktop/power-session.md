...
**The bind depends on the Hyper modifier (#282).** `Hyper+Escape` is `Mod+Ctrl+Alt+Shift+Escape`, which only fires once keyd maps Caps Lock to that chord ([keyd.md](./keyd.md), #282) — land #282 first (or together), or the bind is inert. `Mod+Shift+E` → quit niri is unaffected and remains the keyboard quit regardless. The waybar button is unaffected too (it needs no modifier), so the menu is reachable by mouse even before #282.
