...
**One discoverable surface, one bind — fits the keybind cadence.** [keybinds.md](./keybinds.md) holds a hard cadence: *one bind per learning ceremony*, *no silent additions*. A single menu key exposes all five actions without scattering five raw chords across the keymap (and without binding `systemctl poweroff` to a fat-fingerable key with no confirmation). The menu *is* the "discoverable graphical path" the issue asks for.
...
**A waybar power-button module** — a clickable power glyph in the status bar that opens the menu. Not a competing surface so much as a possible *complement*: it would still call the same menu/script underneath, and it is mouse-only. Out of scope for the keyboard-first capability the issue frames; can be added later pointing at the same script.
