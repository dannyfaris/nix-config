...
The desktop's aesthetic north-star. Living document — the styling tickets that shaped it (display #106, wallpaper #109, pointer/icons #110, compositor styling #111) and the per-surface selection docs (`fonts.md`, `niri.md`, `noctalia.md`; `fnott.md` / `waybar.md` / `fuzzel.md` are kept as record of the decommissioned stack) reference and implement against it, rather than each making locally-sensible but divergent choices. It sets *intent* across every axis of the desktop's look — typography, colour, line weight, radii, spacing, padding, motion, pointer/icons, imagery; the per-surface docs record the specific selections that serve it.
...
| ------------- | -------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **Focus**     | `base0D` | the surface that holds focus        | niri active border, GTK accent, Noctalia `mPrimary`, gh-dash focused border, JankyBorders active (macOS) |
| **Attention** | `base09` | chrome shown *without* taking focus | prompt + statuslines (`.ansi`: untracked count, model tier); no chrome consumer                          |
| **Critical**  | `base08` | error / urgent                      | niri urgent, GTK destructive/error, Noctalia `mError`, gh-dash warning text                              |
| **Muted**     | `base03` | inactive                            | Noctalia `mOutline`, gh-dash faint text + secondary border, JankyBorders inactive (macOS)                |

The boundary is behavioural: a surface uses *focus* if it holds focus when active — which includes overlays like the launcher and the auth prompt, because when active they *become* the focused window, so their accent can never clash with a focus they don't own. *Attention* is for chrome that appears **without** taking focus, coexisting with a separately-focused window and having to read as distinct from it — but no such chrome is Nix-governed today: Noctalia owns the notification surface and decides urgency rendering over the M3 roles the conductor hands it, so no per-urgency slot is declared here (ADR-036; fnott, waybar and fuzzel were decommissioned in #385). The role lives on the terminal bus instead, where the prompt and the agent statuslines spend it on the untracked count and the model-tier label via its `.ansi` projection — `base09` has no ANSI position, so bright-yellow stands in as the nearest on-bus colour. Two assignments are deliberate deviations rather than drift: niri's *inactive* border parks on `base00`, invisible against the background, rather than on *muted*; and the roles reach the desktop through the theme-menu conductor's generated niri/GTK/Noctalia artefacts, not through Stylix targets (ADR-044, #609). The vocabulary is fleet-wide, not niri-only — the same role tokens colour the macOS window border (`modules/darwin/jankyborders.nix`, which cites this section for its role parity). Focus rides base16's canonical accent slot; the other roles are deliberate app-level assignments to tonally-fitting slots — base16 doesn't itself prescribe UI-chrome roles, so this map is ours to hold consistently. On metis the per-host override parks `base0E` on `base0D`'s tone, so the two are equal today and every focus surface reads identically; the discipline is held *by slot* so it stays coherent under any scheme where they diverge. Pursuing Material You (matugen) would mean reconstructing a tonal scheme — matugen does not accept base16 — so it is against the grain and not pursued. **Decided** — implemented in #358; role tokens alias the slots (#369).
...
Cursor theme and application/notification icon theme, cohering with the host polarity and the chrome's restrained mood; cursor *size* driven by the 4K panel. Noctalia owns cursor and icon theming on the Linux desktop (ADR-036, #385), so `stylix.cursor` / `stylix.icons` stay deliberately unwired. **Closed as mooted** (#110). Note the grain of control: cursors are essentially monochrome, so this axis coheres by polarity + size + theme mood, not by base16-palette precision.
...
- `fonts.md` / `niri.md` / `noctalia.md` — per-surface and per-dimension selections that implement this (`fnott.md` / `waybar.md` / `fuzzel.md` are record of the decommissioned stack).
...
- #106 / #109 / #110 / #111 — the styling tickets that shaped this north-star.
