...
**Adopted — a hybrid split:** *mono for the shell chrome you drive (terminal / status bar / launcher — Omarchy-style); sans for content surfaces (notifications / dialogs / web — macOS-style).* Monospace — **Monaspace Argon Nerd Font** (GitHub's humanist mono superfamily; the Nerd Font variant carries the TUI glyphs) — backs the terminal (`foot`) and the TUIs inside it (gh-dash, zellij, starship) *and* the status bar (waybar) *and* the launcher (fuzzel). The mono Nerd Font carries waybar's network/tray glyphs directly, so there is no separate `Symbols Nerd Font` fallback. The content surfaces — fnott notifications, GTK dialogs, and web/document body — render the proportional **IBM Plex Sans** (the IBM Carbon family, cohering with the Carbon spacing scale already adopted). fnott/fuzzel icons come from the icon theme (#110), not the font.

This is the third stance on the question, and it splits the difference between the two before it: the universal-mono setup (#283 / #349 / #351) put mono on *every* chrome surface; the #369 typography pass reversed it to all-sans chrome (mono confined to the terminal); the hybrid keeps sans on content surfaces but returns the *driven* chrome — bar and launcher — to the terminal's mono.

**Sizing is restrained, macOS-style — a flat band of close sizes in regular weights, not a dramatic ramp.** The surfaces sit close together, with the launcher the one deliberately-larger focal element (Spotlight's prompt); hierarchy comes from that focal size and from layout, not from a steep type scale. The sizes are **driven by the display profile** (see the *Hardware is a design input* principle and §"Line weight & radii"): metis runs a **2× niri output scale**, and `lib/display-profiles.nix` couples the scale to the per-surface sizes so they hold a constant apparent size across scales. At metis's 2× profile the band is foot 8 / waybar 10 / fuzzel 11 / fnott + GTK 9 (the 1.5× profile carries the same band one scale up — foot 11 / waybar 13 / fuzzel 14 / fnott + GTK 12 — as the on-vocab reference). Chrome sizes live in `stylix.fonts.sizes` (aliased by the `type.size.*` tokens); a re-tune or scale change is a one-line edit to `display-profiles.nix`.

The sans face is **IBM Plex Sans**, chosen over the other front-runner Roboto (the M3 family's own font): Plex coheres with the Carbon spacing scale already in the design language and reads as distinctive-but-restrained at small sizes, where Roboto reads as ubiquitous. The driven-chrome mono is **Monaspace Argon** (preferred over IBM Plex Mono for its humanist warmth). Per-surface selections and the dpi-aware sizing story live in `fonts.md`. **Decided.**
