...
| **Typography** | Stylix font slots (one mono Nerd Font chrome/TUI/GTK, Inter body). No multi-size *type ramp* adopted — chrome single-size by design. *(Superseded: the typography pass (#369) and the display-scale work (#106) settled a **hybrid** font model — mono for the driven chrome (terminal / bar / launcher), sans (IBM Plex Sans) for content surfaces (notifications / dialogs / web) — sized by a restrained macOS-style band that is **display-profile-driven** at metis's chosen 2× output scale. Not the all-sans chrome an earlier draft of this row recorded.)* | The size band is a flat, restrained set (launcher the one focal element), not a steep M3 ramp; sizes/geometry move with the display profile. | `type.*` referencing Stylix font config. **Typography** dimension. |
