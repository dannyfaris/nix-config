...
- **Beta seams, re-verified per bump.** Five upstream-facing seams — palette schema, `[theme]` keys, the IPC verbs, override semantics, and who writes the polarity signal (below) — are facts of the pinned tag, not upstream commitments. The per-bump ritual (caffeine probe pair + seam re-check + a `nix build` of the package against the pin) is part of any bump, and C0 re-runs on nixpkgs moves too.
...
- **The polarity signal is ours only while the shell doesn't write it — re-check on every bump.** At the pinned beta.4 nothing in Noctalia writes `org.gnome.desktop.interface color-scheme` outside the (disabled) gtk template, which is what lets dconf be the conductor's authority. Upstream `main` has since made the shell write it natively on every palette resolve, templates or not — see `docs/research/noctalia-v5-native-theming.md` §4 correction. Not in any released tag yet; the bump that crosses it turns dconf into an output and leaves a GUI/IPC mode pick unreconciled until the next shell start.
