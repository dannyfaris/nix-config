...
**Stylix target-writer excision on the Linux desktop (E1)** — remove `home/shared/stylix-targets.nix` from the desktop host's `extraHomeModules` (drops the TUI target-writers, incl. the base16-fish OSC palette override) and trim the desktop target-writers (foot, niri) from `home/nixos/stylix-targets-desktop.nix`; GTK keeps its target but layers Noctalia's colours via `stylix.targets.gtk.extraCss` (`@import`), and niri's border rides a runtime `include` (`niri.nix`). **Fonts stay on `stylix.fonts` for now** — the font re-home (off `stylix.fonts` to `fonts.packages` + `fontconfig.defaultFonts`) and dropping IBM Plex are deferred to [#390](https://github.com/dannyfaris/nix-config/issues/390). Keep `stylix.enable` + the host's palette entry as a static colour table for the TUI statuslines. Full literal removal (E2) is deferred — see Sharp edges.
