...
- **Native fullscreen suspends the edge-scroll wrap — accepted as-is.** A native-fullscreen app takes its own Space at the end of the nine, so while it is open the display holds ten. The wrap target is `jq length` over `--spaces --display`, which counts that tenth, giving two behaviours for as long as the app stays fullscreen:
  ...
  Nothing breaks and no state is lost; the wrap simply stops working until fullscreen is exited, and the plain step keeps working throughout. The operator confirmed both behaviours and accepted them (2026-08-11) rather than guarding them in code — the case only arises while you are inside the fullscreen app anyway. The fix, if it is ever wanted, is to filter `."is-native-fullscreen"` out of the count in `home/darwin/skhd.nix`'s `spaceCount`, which would restore the wrap at the cost of a longer query in every edge-scroll press.
