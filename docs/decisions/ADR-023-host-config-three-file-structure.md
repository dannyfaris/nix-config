...
**Banner enforcement** (added per [ADR-025](./ADR-025-ci-in-flake.md)): the auto-generated-banner rule is enforced by a pre-commit hook in `parts/checks.nix` that invokes `scripts/hardware-config-banner.sh` against any staged `hosts/<name>/hardware-configuration.nix`. The hook is installed automatically via `nix develop` (or direnv-reload — `git-hooks.nix` writes `.git/hooks/pre-commit` and clears any stale `core.hooksPath`); CI runs the same check via `nix flake check`. `hosts/nixos-vm/hardware.nix` is not matched by the regex — the legacy two-file exception remains unenforced by design. Statix and deadnix lint rules are similarly carved out for hardware-config files (via `statix.toml` and the per-hook excludes in `parts/checks.nix`), and the treefmt formatter is told to skip them via `treefmt.settings.global.excludes` in `parts/formatter.nix`, so a future `nixos-anywhere --generate-hardware-config` regenerate produces a clean drop-in commit.
