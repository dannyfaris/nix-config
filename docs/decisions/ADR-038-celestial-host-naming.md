...
| `nixos-vm` | **Triton** | moon (of Neptune) | UTM/aarch64 refinement VM, pinned to the home Mac |

`nixos-vm` runs in UTM on the Mac mini (→ Neptune), so it is a moon of Neptune → **Triton** (Neptune's largest moon). The name now encodes the dependency instead of hiding it.
...
3. **Remaining hosts** — `metis` → Mars; `nixos-vm` → Triton (the moon of Neptune); and the two new machines (Jupiter, Saturn) — land as they are touched, on the same pattern.
