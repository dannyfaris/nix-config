---
date: 2026-05-28
status: Amended by ADR-029 (item 3 retracted; items 1–2 stand and are implemented); §Decision item 1 further amended by ADR-036 (Stylix no longer the Linux-desktop theming source of truth), its TUI clause ended fleet-wide by [ADR-041](./ADR-041-terminal-authority-tui-theming.md) (TUIs follow the terminal palette), and item 1 **retired on the NixOS side entirely** by §History (2026-08-21) — Stylix is Darwin-only; item 2 (the desktop-env bundles) stands
---
...
Every consumer of `config.lib.stylix.colors` in this repo — helix, prompt, zellij, the agent statuslines, macchina, the metis desktop surfaces — maps semantic roles to base16 *slots* and assumes each host's scheme renders the slots' intents. [#331](https://github.com/dannyfaris/nix-config/issues/331) documents (with verified hex evidence) that two of the four host ports violate that assumption in the consumer-visible accent range (08–0E): the tokyo-night ports on mercury park the theme's red in `base0F` and render 08/09/0A off-intent (dark `base09` is byte-identical to the `base05` foreground), and the rose-pine ports on metis have six accents for seven used slots, shipping the forced duplicate as `09==0E`.

**Policy.** Host schemes must render base16 slot intents across 08–0E. Where a community port fails this, the correction is per-host: `lib/host-palettes.nix` entries gain an optional `overrides` field (per-polarity attrset of slot → hex) which both `stylix-palette.nix` modules pass to `stylix.override`, merging over the selected scheme at `config.lib.stylix.colors` generation. Corrective hues are imported only from the theme's own upstream palette, so corrections never erode the cross-host palette-shift signal this file exists to provide. Where a palette has fewer accents than slots, the forced collision is placed deliberately rather than inherited from the port's accident.

**Mercury: surgical override over port swap.** The in-package conventional port (`tokyo-night-terminal-dark`) was rejected on measurement: its `base05` body text scores 4.40 contrast against its background — below the WCAG AA 4.5 threshold — versus 8.10 for the current port, on a headless host that is only ever read as text; it also imports unrelated regressions (a teal `#41A6B5` in the green slot, swapped 00/01 backgrounds). The override seam is required for metis regardless (no alternative rose-pine port exists), so the swap's no-new-mechanism appeal nets to zero. Corrections: dark `08 ← f7768e` (the red from this port's own `base0F`), `09 ← ff9e64`, `0A ← e0af68` (upstream orange/yellow, as shipped in the sibling terminal port); light `08 ← 8c4351` (own `base0F`), `0A ← 8f5e15`; light's `base09` (`965027`) is already on-intent and deliberately left alone.

**Metis: the unavoidable collision moves to `0D==0E`.** `base0E ← iris` (`c4a7e7` dark / `907aa9` dawn). The SSH host marker becomes purple-family, matching the other three hosts — the fleet's cross-host constant signal now reads constant — and `09`-vs-`0E` becomes distinct on every host, unblocking the model-tier colour selection deferred from #330. Cost accepted: when SSH'd into metis, the host marker and the adjacent path share iris (glyph- and text-distinct; a coherent "location-family" pairing — gh-dash's accent already rides `0D` iris). Rejected placements: status quo `09==0E` (fleet-inconsistent SSH signal; blocks the tier-colour decision); `0A==09` and `0C==0D` (each merges two elements that sit adjacent in the same segment); any pairing involving `08` (dilutes the alarm slot).

**Consequences.** Mercury's and metis's rendered palettes are no longer byte-identical to their ports — the pinned corrections persist across upstream scheme updates, which is the point. Conformant hosts (nixos-vm, mac-mini) pass an empty override and are byte-for-byte unchanged. Verification is eval-level (asserting `config.lib.stylix.colors` outputs for corrected and conformant hosts) plus an on-host visual pass on mercury and metis across the consumer surfaces.

### Amendment (2026-08-03, #331 close-out) — re-scoped to per-family, post-#634 fleet; enforcement verified at the rendered-artifact level

The 2026-06-10 entry above is preserved as history; this entry supersedes its host framing and its mechanism description, and records the verification that closes #331. Three axes drifted after that entry and are corrected here.

**Per-family, not per-host.** The 2026-06-10 text framed the corrections as *per-host* and pointed at a `lib/host-palettes.nix` overrides field. Host-identity theming was retired on 2026-07-13 (operator call; fuller rationale lands with [#609](https://github.com/dannyfaris/nix-config/issues/609)'s authority ADR): the palette catalogue is now global, and `lib/host-palettes.nix` is dead. The live mechanism is **per-family** slot overrides declared once in `lib/theme-families.nix`, threaded through `lib/palette-for.nix` (#541) into *both* the boot-default palette (the `stylix-palette.nix` twins → `stylix.override`) and every runtime conductor variant (`lib/scheme-pair.nix`'s `mkVariant` replays the same `.override`). A correction is therefore intrinsic to the family — any host selecting that family at runtime inherits it — not attached to a host. `mercury` and `nixos-vm` are decommissioned ([#634](https://github.com/dannyfaris/nix-config/issues/634)); `alcyone` is tokyo-night's boot host now, `metis` remains rose-pine's.

**The invariant, re-expressed per family.** *Every catalogue family must render base16 slot intents across 08–0E in both polarities.* This is the fleet-wide invariant #330 binds to (its model-tier colour needs a slot that honours a distinct intent on every family), and it is the sentence the tier-colour follow-up should cite. Two families are defective-and-corrected at the pin; three are audited-conformant:

- **tokyo-night — defective, corrected.** The ports park the theme's red in `base0F` and render 08/09/0A off-intent (dark `base09` is byte-identical to the `base05` fg; light `base08` is one hex digit off `base05`). Corrected in both polarities from the theme's own upstream palette: dark `08←f7768e` `09←ff9e64` `0A←e0af68`; light `08←8c4351` `0A←8f5e15` (light's `base09` `965027` is already on-intent, left alone).
- **rose-pine — defective, corrected.** Six accents for seven used slots forces one collision; the port ships `09==0E` (gold). Relocated to `0D==0E` (iris — `c4a7e7` dark / `907aa9` dawn) so `0E` stays purple-family and `09` is freed. The collision is *placed*, not eliminated: a consumer reading `0D` and `0E` as distinct roles still collides on rose-pine (flagged, not a regression).
- **catppuccin, gruvbox, solarized — audited-conformant, empty override.** All accent slots 08–0E render distinct, conventional hues in both polarities; none equals `base05`. catppuccin was audited under its former nixos-vm binding; solarized (alnair/saturn boot default) was newly audited this pass and verified clean (`08←dc322f` red, `09←cb4b16` orange, `0E←6c71c4` violet, all byte-distinct).

**Pin-validity caveat.** This audit is valid at the current transitive `base16-schemes` pin (through nixpkgs). An upstream re-port can shift slot values and silently un-match an override; re-verify the defective families and re-audit the conformant ones after any nixpkgs bump.

**Verification record.** Enforcement is confirmed at the *rendered-artifact* level (not merely eval — "set ≠ enforced", [#303](https://github.com/dannyfaris/nix-config/issues/303)) on both delivery paths (boot-default palette and runtime conductor variant) and on two live hosts: metis rose-pine artifacts (2026-07-14); alcyone tokyo-night + rose-pine artifacts (2026-08-03, probe run from neptune over the declared SSH edge, read-only, adversarially reviewed before execution). The corrected hues appear in the rendered surfaces — foot regular/bright, gtk destructive/error/accent, niri urgent-color, noctalia mError/mPrimary/terminal blocks — family-intrinsically, on the second live host confirming the per-family delivery. **base09 negative control:** `ff9e64` (tokyo-night's `base09` correction) is absent from every rendered Linux artifact — because no theme-menu renderer references `base09` at all; its only reachable surface today is `lib/theme-tokens.nix`'s `.ansi` projection, where `attention` collapses to bright-yellow (the base16→ANSI bus carries no `base09` position). The corrected value is real at the eval layer and inert at every current consumer — the fact the #330 follow-up must decide against. **The subjective on-display visual pass is waived with record:** metis is headless, the family corrections are host-independent, and the artifact evidence carries the enforcement claim; an opportunistic eyeball on alcyone remains available but gates nothing (operator ruling, 2026-08-03).

### Amendment (2026-08-21, #885) — Stylix exits the NixOS side entirely; item 1 is now Darwin-only

**What changed.** Decision item 1 promoted Stylix to `modules/nixos/foundation.nix` as the fleet's theming source of truth. That is retired for NixOS. `modules/nixos/stylix-palette.nix` and `home/nixos/stylix-targets-desktop.nix` are deleted, foundation's import goes with them, and `home/shared/stylix-targets.nix` — which survives, empty, for its whitelist stance — is reached only by celaeno. `grep -rl stylix modules/nixos home/nixos` returns nothing. Darwin is untouched: `modules/darwin/stylix-palette.nix`, the ADR-044 conductor, `lib/scheme-pair.nix` and the Ghostty target all stand, and celaeno's system derivation is **bit-identical** across the change (same `.drv` hash). The `flake.nix` stylix input stays for them.

Everything the engine still did on Linux was a first-class home-manager option, so each moved to the option directly, values unchanged: the GTK toolkit theme, the `gtk4` theme and the dialog font in a new `home/nixos/gtk.nix`; the icon theme and the pointer cursor in `home/nixos/pointer-icons.nix`, where a shared let-binding now feeds both `home.pointerCursor` and niri's cursor block so the two cannot drift. `modules/nixos/desktop-fonts.nix` loses its `stylix.fonts` block; the one live value in it (the popups font size) re-points to its real source, `lib/display-profiles.nix`. Coverage is 1:1 at the artifact level, not by inspection: alcyone's home-manager files derivation is byte-identical to the pre-change build except for the two removals below. `lib/theme-tokens.nix` splits — the config-free surface moves to `lib/static-tokens.nix`, and the `{ config }` half keeps the name, imports the static half and merges the resolved hexes over it, so its one remaining consumer (`modules/darwin/jankyborders.nix`) sees an unchanged surface while Linux consumers import the static file and cannot force a Stylix eval by construction. `lib/theme-families.nix`'s three NixOS host entries are dropped as dead data.

**Why.** Post-G6 (#825) the engine was kept alive to do three things home-manager does natively, while dragging a mandatory base16 palette that nothing on Linux rendered — colour there is Noctalia's, at runtime, per [ADR-048](./ADR-048-noctalia-theming-delegation.md). The palette was not merely inert: every NixOS host, headless electra included, shipped `/etc/stylix/palette.{html,json}` and paid an import-from-derivation build of `base16-schemes` at **every eval**. A boot-default palette with zero renderers is not a cheap thing to keep for symmetry's sake.

**Consequences, honestly.** A fresh Linux host renders **tool defaults** until the first Noctalia theme pick — ratified rather than replaced, because that is already what happens today with the palette present (foot: empty seed since #385; GTK: no `gtk.css` until the first resolve, #874; niri: optional include absent; TUIs: the terminal palette). The boot palette painted nothing on first boot, so removing it changes nothing observable; re-adding machinery to paint a one-pick-long window on the way *out* of an engine would fail [ADR-032](./ADR-032-proportionate-enforcement-and-rationale.md). Shape stays declared throughout — adw-gtk3, fonts, icons and cursor persist via their native options. The **icon light/dark pair collapses to a single dark literal** (`Colloid-Dark`): `stylix.icons` carried the couplet, `gtk.iconTheme.name` takes one name, every host is dark polarity, and wiring a pair mechanism with no consumer is the same ADR-032 failure. A light-polarity host re-opens it. Two artifacts disappear from every NixOS host — the `/etc/stylix` and `~/.config/stylix` palette dumps — and a third from the desktop hosts only: the polkit agent's `gtk.css` restart trigger (which resolves [#876](https://github.com/dannyfaris/nix-config/issues/876), and was the only reason Stylix's never-linked `gtk.css` render sat in alcyone's closure). Every theme package the residue existed to carry — adw-gtk3, colloid, phinger, the fonts — stays at an identical store path.

**Record.** No successor ADR, and no `docs/design/` note: the design record is [issue #885](https://github.com/dannyfaris/nix-config/issues/885), whose comments carry the four settled stages. Both are deliberate operator rulings against the repo's own conventions — the reversal-gets-a-successor pattern (035→044, 038→045, 040→047, 044→048) is not followed here, and the design-note convention is departed from as a recorded experiment, not drift. This entry plus the status line above are the durable record.
