...
**Explicit triggers, no passive watch.** niri's inotify misses symlink swaps (niri#2658, NixOS-specific — the inotify watch is on the file, not the target; a swap changes the link's target in-place but doesn't CREATE a new file event). Noctalia's FileView monitors `watchChanges: true` on the **parent directory** — so the `ln -sf` that re-points `$stateDir/colors.json` to a new store path does fire a directory-change event (confirmed from source). No passive file-watch was assumed for any surface without source verification.
...
- ✗ The live-repaint plumbing (foot OSC loop, niri msg, colors.json swap) is ours to maintain, not Noctalia's.
