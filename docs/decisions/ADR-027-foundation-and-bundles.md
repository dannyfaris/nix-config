...
### Storage-integrity capability bundle (2026-08-15)

`btrfs-scrub.nix` had drifted into being fleet-wide *by convention* — three host files independently importing it, with nothing making that structural — and #847's new `disk-health.nix` was about to inherit the same shape. #854 asked whether such guarantees belong in `foundation.nix` instead.

They do not, and this ADR already said so: the Decision's "a capability that every host of a platform happens to want today still belongs in a capability bundle, not in foundation; fleet uniformity is a snapshot property" is exactly this case, and the Rationale's "reserved for things that *cannot be opt-out capabilities*" excludes it on its own terms — a ZFS host would legitimately decline the btrfs half, which Atlas ([#642](https://github.com/dannyfaris/nix-config/issues/642)) will be. The counter-argument considered and rejected was an inertness criterion (admit a module to foundation if it is universal in intent and provably inert where inapplicable — `autoScrub` with no btrfs filesystems creates zero units). It was not needed once the two modules were judged separately rather than as one class.

So the two modules become `modules/nixos/bundles/storage-integrity.nix`, imported by the three NixOS hosts, satisfying the ≥ 2 members convention from birth. No decision here changes; this is an application of the taxonomy this ADR already defines, recorded because the question will recur as the fleet's storage tier grows.
