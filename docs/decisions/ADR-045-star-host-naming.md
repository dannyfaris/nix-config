...
3. **Full fleet re-key, no grandfathering.** A half-planets / half-stars fleet is the incoherent namespace the theme exists to prevent. This is the cheapest the re-key will ever be: of ADR-038's planet renames only `neptune` (the #403 pilot) landed as a deployed host; `saturn` also carries a planet name in-repo but its hardware is not yet deployed (#639), so re-keying it is a trivial repo-only edit. Re-keying now touches barely-started ground rather than locking in a permanent mixed theme.
