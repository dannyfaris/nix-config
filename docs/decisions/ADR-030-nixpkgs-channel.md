...
- **Reduced freshness for agent-CLI tooling.** The day-to-day workflow leans on `claude-code`, `cursor`, `mosh`, `helix`, `nh`, and friends — packages whose unstable cadence delivers material fixes (not just version churn). Stable would lag these by months.
