...
| [038](./ADR-038-celestial-host-naming.md) | Host-naming framework | celestial bodies; substrate picks class | **Accepted, Implementation pending.** Owned metal = moon-capable planet, VPS = moonless planet, pinned VM = a moon of its host's planet, roaming/misc = the minor-body reserve; gravitational binding mirrors operational dependency. Re-keys the fleet (Jupiter/Saturn/Mars/Neptune/Mercury; `nixos-vm` reserve-typed, Triton withdrawn per #448), staged per-host. Amends ADR-016 (supplies the naming source; overturns its one-time in-place-rename clause). |
