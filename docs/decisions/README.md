...
| [035](./ADR-035-runtime-theme-polarity.md) | Runtime theme/polarity (metis) | tinty delta over the Stylix base | **Proposed.** First sanctioned runtime/session-state layer: tinty selects theme+polarity at runtime over Stylix's build-time base (depth, fonts, boot default, Nix-pinned menu). Polarity flips the reachable surface near-instantly via IPC/signal/portal; arbitrary-scheme is terminal/TUI-only. Materialise-real-file primitive (ADR-024 lineage) for niri; metis-only. Resolves the design half of #143. |
| [036](./ADR-036-doc-mutability-contracts.md) | Documentation contracts | facts as code, reasons as prose | **Accepted, Implementation pending.** Classifies every doc artifact (incl. CLAUDE.md) by mutability: facts generated/checked, reasons single-sourced prose, decisions in the tracker, only load-bearing why a frozen ADR. Grep-lint rung committed; heavier enforcement on evidence. |
