...
| [016](./ADR-016-host-identity.md) | Host identity | stable per physical machine | Hardware change = new host; software role change = no rename. **Amended by ADR-038** (naming source is now the celestial theme; the one-time in-place re-key clause overturned — identity rule itself stands) |
...
| [028](./ADR-028-stylix-foundation-and-desktop-env.md) | Theming + desktop env | Stylix in foundation; desktop arrives on metis | **Amended by ADR-029** (item 3 retracted; items 1–2 stand and are implemented), **ADR-036** (§item 1 — Stylix no longer the Linux-desktop source of truth) **and ADR-041** (§item 1 TUI clause ended fleet-wide — TUIs follow the terminal palette; Stylix stays as colour table + palette engine). Niri + Foot + greetd. |
...
| [034](./ADR-034-no-host-data-backup.md) | Host-data backup | out of scope — reprovision-from-source | Deliberate non-adoption: no restic/borg/snapshots. Recovery from host loss is reprovision-from-flake plus the homes durable data already has (git, 1Password, cloud, the off-band sops identity). Only metis `/home` + neptune home hold non-reproducible data; the residual (uncommitted WIP) is accepted. Migration trigger if irreplaceable local-only data accumulates. |
...
| [038](./ADR-038-celestial-host-naming.md) | Host-naming framework | ~~celestial bodies; substrate picks class~~ | **Superseded by ADR-045.** Owned metal = moon-capable planet, VPS = moonless planet, pinned VM = a moon of its host's planet, roaming/misc = the minor-body reserve; gravitational binding mirrors operational dependency. Retired when the planet pool hit its ceiling and "moon-capable" went dishonest for VM-less machines — the name carried an operational claim. Amends ADR-016 (naming source; one-time in-place-rename clause), re-affirmed by ADR-045. |
| [039](./ADR-039-capability-registry.md) | Capability registry & interaction architecture | single-source `lib/capabilities.nix` → every surface | **Accepted, Implementation pending.** One semantic registry generates the keybind configs (niri/keyd/Karabiner/Hammerspoon), the unified action/cheatsheet palette, and the `keybinds.md` table from one declaration per capability. Freezes the Hyper taxonomy (`Ctrl+Alt`/`Ctrl+Opt` base, parity-not-identity, `Super` = Cmd-parity), the three-dimension schema (chord-tokens · realization · descriptive metadata), the atomic base-shape cutover, the unified palette, and the pure-Hammerspoon macOS realization (resolves #440). **§7 (macOS realization) superseded by ADR-040** (AeroSpace WM); the registry architecture stands. Detail single-sourced to keybinds.md + the research notes + #384/#437/#442/#440. |
| [040](./ADR-040-macos-window-manager-aerospace.md) | macOS window manager | AeroSpace (tiler owns workspaces) | **Accepted, Implementation pending.** Supersedes ADR-039 §7 (pure-Hammerspoon/native-fullscreen → AeroSpace-owned workspaces) after a live Phase-A trial (GO). Single native Space, i3-flat tiling, no SIP; declarative `pkgs.aerospace` + `programs.aerospace`; binds single-sourced via a new `aerospace-action` emitter; **Hammerspoon retired entirely** (Ghostty `open -na` spike) → Karabiner + AeroSpace only; JankyBorders + token-sourced gaps. The *why* lives in the design note `macos-deterministic-tiling.md`; Phase B is #494. |
| [041](./ADR-041-terminal-authority-tui-theming.md) | TUI colour authority | terminal ANSI palette (Stylix targets emptied) | **Accepted, Implemented.** TUIs render from the terminal's 16-colour palette (bat `base16`, helix `base16_terminal`, fzf `--color=16`, zellij `ansi`, starship/lazygit native names, yazi dual presets) instead of build-time Stylix hex — they follow runtime polarity flips (#499) and the local palette over SSH. Ends ADR-028 item 1's TUI clause fleet-wide; retires the per-host palette-shift SSH signal for TUIs (operator-endorsed); token roles gain an `.ansi` projection (`lib/theme-tokens.nix`) for terminal-following surfaces. Stylix stays as colour table (#411 statuslines) + palette engine (scheme-pair, Ghostty). |
| [042](./ADR-042-fleet-ssh-declared-edges.md) | Fleet SSH trust topology | declared edge whitelist; static per-host keys stay | **Accepted, Implementation pending (#524 re-scoped).** Amends ADR-010. `sshEdges` (destination → sources) in `lib/` derives per-host `authorizedKeys`, stance-asserted (+#551 probe rung later); SSH CA and Tailscale SSH declined on pin-verified evidence, four ⚠ migration triggers recorded. Target fleet: sources = the three interactive workstations (jupiter/neptune/saturn); service tier pure sink — trust flows downhill, sinks hold no outbound keys. saturn gains inbound (deliberate stance flip); mercury and nixos-vm retired, #634. The *why* lives in the design note `fleet-ssh-identity.md`. |
| [043](./ADR-043-fleet-key-custody.md) | Fleet key custody | host-derived machine keys + standalone operator root | **Accepted, Implemented (#526).** Neptune's triple-role `id_ed25519` (fleet SSH + GitHub push + sops recovery root) retired via the executed rotation. Machine sops decryption = host-key-derived age identities, one per NixOS secret-holder (Darwin holds none — no secrets declared); edit + recovery root = one standalone operator age key, vault + offline copy, atomic rotation across copies; recipient set = {live NixOS secret-holders} ∪ {operator}. Constraint on #557 (seal the disk, never TPM-bind the host key); non-revocation of dropped recipients accepted with `sops rotate` + value rotation as the compromise escalation. The *why* lives in the design note `fleet-key-custody.md`; rotation record in ADR-010 §History. |
| [044](./ADR-044-linux-runtime-theme-menu.md) | Linux runtime theme menu | Nix-owned per-entry derivations + symlink swap | **Accepted, Implemented (#609).** Nix/Stylix is the single theming authority on the Linux desktop; a Nix-declared catalogue renders foot/niri/GTK/colors.json artefacts per family at build time; a `current` symlink + per-target resolved symlinks are the only runtime state; the `theme` CLI switches them atomically with explicit reload fan-out (foot OSC pty loop, niri msg, dconf portal write). Noctalia demoted from colour authority to themed-by-Nix shell — **§item 3 of [ADR-036](./ADR-036-noctalia-shell-linux-desktop.md) reversed**. Host-identity theming retired (boot defaults only; full catalogue available fleet-wide). Two-axis persistence: family = symlink (survives reboot); polarity = dconf GVDB. Runtime-verified on metis 2026-07-14. |
| [045](./ADR-045-star-host-naming.md) | Host naming (revised) | stars; flat pool, non-binding | **Accepted, Implementation pending.** Supersedes ADR-038: hosts named after stars, a flat pool that makes no operational claim, so the planet ceiling and the substrate-vs-practice dishonesty both dissolve. Full fleet re-key, no grandfathering (only `neptune` deployed under a celestial name). VMs are just stars (moon→planet binding dropped). Collective fleet named **Sidera**. The living rule + roster + southern-sky flavour live in taxonomy.md (non-binding, deliberately not frozen here); re-affirms ADR-016's one-time rename licence. |
| [046](./ADR-046-markdown-formatter.md) | Markdown formatter | dprint (`textWrap:never` via treefmt) | **Accepted, Implementation pending (wired in #435 PR B).** Mechanises the soft-wrap convention: dprint's markdown plugin (`textWrap:never` + `emphasisKind:asterisks`, hermetic wasm plugins) through the same treefmt seam as nixfmt/shfmt; the one-off tree-wide reflow lands with the wiring — **reverses workflow.md's "no tree-wide reflow" reservation**. ADR `Date`/`Status` headers moved to values-only YAML frontmatter ahead of the reflow (#441 owns the schema). Declined: Prettier (noisier reflow), mdformat (weak GFM-table story), `.git-blame-ignore-revs`, dprint in packages/dev-shell. |
