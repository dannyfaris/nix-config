...
The Decision and Implementation sections' hook enumerations ("Hooks: nixfmt, statix, deadnix, actionlint, …") predate this change and are read as historical; the live hook set is `parts/checks.nix`. Note also that those enumerations already listed *nixfmt* rather than *treefmt* as the formatter unit — another reason to treat the prose lists as point-in-time, not current.
