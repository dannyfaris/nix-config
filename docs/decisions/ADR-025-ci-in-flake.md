...
**The mechanism avoids the gotcha the original stance was built around, rather than accepting it.** §60's "Always run; no `paths-ignore`" rejected skipping on two grounds: required status checks that are expected-but-never-reported leave a PR permanently unmergeable (silently, and forever, under squash auto-merge), and the YAML needed to dodge that cleanly was not worth the savings. The first objection stands entirely — it is exactly why nothing is skipped here. The `flake-check` job always runs, its matrix always expands, and all three `flake-check (<arch>)` contexts always report under their existing names. Only the *command* changes: on a docs-only PR the job builds a new `checks-without-hosts` flake output — every check except the per-host `system.build.toplevel` builds, derived by *excluding* the `host-` prefix so future checks are picked up without an allowlist — after a `nix flake check --no-build` schema pass, and the cache step is skipped so `purge-primary-key: always` cannot resave a docs-only store over the warm cache. Because the cheap set still contains the pre-commit hooks and the doc-freshness gates, no per-document carve-outs exist to maintain. Standing mechanics live in [docs/ci.md](../ci.md) §"Docs-only short-circuit"; the options weighed (including the job-level `if:` and stub-workflow shapes, both rejected for re-entering the same hazard) are in [docs/design/ci-docs-only-short-circuit.md](../design/ci-docs-only-short-circuit.md).
