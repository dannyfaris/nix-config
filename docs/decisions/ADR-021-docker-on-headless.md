...
- ⚠ Migration trigger: a future workflow that hits the `docker compose`-subcommand-vs-`docker-compose`-standalone distinction in surprising ways. The cli-plugins auto-discovery in modern nixpkgs handles this well at the time of writing; if it stops working, falling back to `docker-compose` (standalone) is the obvious patch.
