...
**Host side.** The same constraint binds every machine that evaluates the flake — each fleet host's rebuilds, and the operator machine during a bootstrap. There the wiring is a user-scoped `access-tokens` line in `~/.config/nix/nix.conf`, derived from the host's own `gh auth` session; the one-liner and its caveats live in both bootstrap runbooks (§Operator prerequisites in [darwin-bootstrap](./runbooks/darwin-bootstrap.md) and [headless-bootstrap](./runbooks/headless-bootstrap.md)). If per-host wiring proves friction, the named alternatives are a sops-managed netrc behind `nix.settings.netrc-file` (declarative, fleet-wide, one secret) or flipping the input public — both weighed in the wiki repo's `docs/design/deployment-packaging.md` (its fork-2 ruling); re-rule there, not here.
...
<!-- probe: #703 stale-base verification (choreography step 2, M0); never merged -->
