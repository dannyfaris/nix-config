...
**Rule.** All markdown in the repo — docs, ADRs, this file, READMEs — is soft-wrapped: one line per paragraph, no hard newlines mid-paragraph. Tracked `.md` files are formatted to this shape by dprint via treefmt (`nix fmt`), the mechanism selected in [ADR-046](./decisions/ADR-046-markdown-formatter.md) and wired in #435 PR B; before that wiring lands the shape is authored by hand. Issue and PR *bodies* stay hand-authored soft-wrapped — the formatter cannot reach `gh` descriptions. List items, table rows, and fenced code keep their natural line structure. This is true soft-wrap, not semantic line breaks (one sentence per line).
...
- The formatter (dprint via treefmt, once wired) reflows every tracked `.md` to one line per paragraph on `nix fmt`; the one-off tree-wide reflow rides #435 PR B. The earlier "no tree-wide reflow PR" reservation is retired — mechanizing the convention makes the one-off reflow the point, not a hazard.
