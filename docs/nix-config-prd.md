...
| 6 | `hosts-build`         | Every host configuration that the current machine can build natively, builds. There is no cross-platform build path — each platform's hosts are built on that platform (in CI, its own matrix leg).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `nix flake check` (per-host `toplevel`) + CI |
...
| CI (GitHub Actions) | `nix flake check` on one matrix leg per platform the fleet has hosts on — today `x86_64-linux` and `aarch64-darwin` — on every push and pull request, except on documentation-only pull requests, which build every check *but* the host toplevels (see [ci.md](./ci.md) §"Docs-only short-circuit") | minutes            |
...
Continuous integration is live (per [ADR-025](./decisions/ADR-025-ci-in-flake.md)). GitHub Actions runs `nix flake check` on every push and pull request, with **one matrix leg per platform the fleet has hosts on** — today `x86_64-linux` and `aarch64-darwin` — building every host on a native runner of its own platform, except on documentation-only pull requests, which build every check *but* the host toplevels (see [ci.md](./ci.md) §"Docs-only short-circuit") — defined in `.github/workflows/ci.yaml`. `aarch64-linux` is a supported platform (§2.1) but has no host today, so it has no leg; the matrix tracks hosts, not the platform list. A weekly `flake-lock.yaml` job opens a lockfile-bump PR that merges manually after green CI. PRs land via squash auto-merge once required checks pass. CI is thin YAML; every check is a flake output, so local `nix flake check` and CI verify the identical set.
...
- **Build.** Native builds via `nix build .#<host>` confirm each host configuration produces a valid system closure. There is no cross-platform build path, so each host is built on a native runner of its own platform — locally, and in CI.
...
**macOS hosts (managed via `nix-darwin`):** Install Nix on the existing macOS install, clone, activate via `nix run nix-darwin -- switch`. After first activation, `darwin-rebuild` is available on `PATH` and the canonical command is `nh darwin switch`. The operational sequence — including the SSH-keypair carry-over, age-identity derivation, and post-activation `~/.ssh/config.local` authoring — is captured in [`docs/runbooks/darwin-bootstrap.md`](./runbooks/darwin-bootstrap.md). MacOS-native applications are managed declaratively via `nix-homebrew` and nix-darwin's `homebrew` module per [ADR-031](./decisions/ADR-031-nix-homebrew-boundary.md): Homebrew casks for vendor-targeted Standalone builds (Ghostty, Tailscale, 1Password); Mac App Store apps via `homebrew.masApps` for clause-3 carve-outs where MAS's update path is the named advantage (Slack). `masApps` carries a cleanup asymmetry — entries dropped from the attrset are not auto-uninstalled (Homebrew Bundle limitation); per-tool docs under [`docs/desktop/`](./desktop/) record the manual `mas uninstall <id>` step. A one-time interactive App Store sign-in per machine is required before the first `masApps` activation succeeds (captured in the runbook).
...
**Continuous integration.** *Resolved by [ADR-025](./decisions/ADR-025-ci-in-flake.md):* CI is now live (§9.5) — GitHub Actions runs `nix flake check` on one matrix leg per platform the fleet has hosts on (today `x86_64-linux` / `aarch64-darwin`) on every push and PR, building every host on a native runner of its own platform. Was deferred here pending demonstrated cross-platform-coverage value.
