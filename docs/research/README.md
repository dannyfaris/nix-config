...
- **[noctalia-v5-palette-seam-review.md](./noctalia-v5-palette-seam-review.md)** — adversarial review of the candidate architectures for re-wiring ADR-044's themed-by-Nix seam against Noctalia v5 (v4's watched `colors.json` has no v5 read path): the catalogue-as-`customPalettes` + IPC design rejected on the settings.toml authoritative-duplicate finding, the mutable-file port steelmanned, and the adopted constant-name symlink hybrid. Feeds [`../design/noctalia-v5-migration.md`](../design/noctalia-v5-migration.md) (#644).

- **[noctalia-v5-idle-sleep-review.md](./noctalia-v5-idle-sleep-review.md)** — adversarial review of the idle→suspend + lock-before-sleep design for metis: the caffeine-doesn't-gate-idle upstream bug (F1), the sleep-delay-inhibitor gap and `systemd-lock-handler` fix, remote-activity blindness and the guard-script mitigation ranking, WoL/wake findings, and side-findings (`LockedHint` unusable, `HandlePowerKey=poweroff`). Feeds [`../design/noctalia-v5-migration.md`](../design/noctalia-v5-migration.md) (#644).

- **[noctalia-v5-derisk-audit.md](./noctalia-v5-derisk-audit.md)** — a third-pass audit of the migration design note's own De-risk section: every source claim re-verified at the tag (all reproduced), the stretched/misattributed wordings corrected, coverage gaps found (the C0 build-against-pin gate — passed, see addendum; the C2/started-hook contradiction; the SSH socket-guess design gap), and the probe plan sharpened. Feeds [`../design/noctalia-v5-migration.md`](../design/noctalia-v5-migration.md) (#644).

- **[noctalia-v5-native-theming.md](./noctalia-v5-native-theming.md)** — source-level evidence on Noctalia v5's native theming engine (colour sources incl. the vendored material-color-utilities port and hand-written terminal blocks, the 20-template catalog with its config-injection apply scripts, full config.toml declarability, the no-pin settings.toml persistence model, portal/polarity behaviour, wallpaper coupling), gathered for the question of migrating the Linux theming authority off the ADR-044 conductor. In-session assessment: no migration at beta.4 (the no-pin selection conflict + churn); hybrid landing zone recorded for any future revisit. Feeds ADR-044's context and #648.

- **[ephemeral-root-recovery-workflows.md](./ephemeral-root-recovery-workflows.md)** — targeted three-agent translation pass (snapper/openSUSE · ZFS boot environments · btrfs-native) turning mature recovery workflows into the archived-root runbook for #553: the three-verb shape (browse / reflink-extract / snapshot-into-place promote), `set-default` and boot-menu `rootflags` ruled out with mechanisms, the promotion×per-boot-wipe interaction (kill-switch as recovery infrastructure), the Nix-GC×retention coupling, and the nested-subvolume rename-vs-snapshot contradiction adjudicated. Feeds [`../design/ephemeral-root.md`](../design/ephemeral-root.md).

- **[ephemeral-root-prior-art.md](./ephemeral-root-prior-art.md)** — deep-research sweep of ephemeral-root ("erase your darlings" / impermanence) practice on NixOS: mechanism landscape (tmpfs vs btrfs blank-snapshot rollback vs the impermanence module, with initrd-timing failure stories), old-root retention practice (rarer than assumed; no documented recovery runbook in the fetched set), a convergent seed persist whitelist, `/home` handling in the wild (persistent `/home` is the originating baseline; ephemeral `/home` is a singular counterexample), and audit/declaration-discipline prior art (`nix eval`-driven undeclared-state diffing ≈ #633's probe; module-owns-its-state has no antecedent). Feeds #553 / #633 / #631.
