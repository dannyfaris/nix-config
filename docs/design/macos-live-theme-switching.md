...
**Polarity authority is the macOS appearance state.** `AppleInterfaceStyle` *is* the runtime polarity pointer — not a file we invent. Every entry point (the System Settings toggle, Control Centre, a CLI switcher, the future action-menu entry per ADR-039's capability registry) converges by writing that one preference; macOS then fires `AppleInterfaceThemeChangedNotification`, and a single watcher path fans out from there. Because the switcher *only* writes the preference and the watcher is the only fan-out driver, there is one code path and no notification loop to guard — the loop-guard #499 flagged dissolves structurally. One caveat carries to implementation: not every write path fires the notification — a bare `defaults write -g AppleInterfaceStyle` neither notifies nor applies live, so the switcher must use a path that does (the System Events AppleScript route, which needs a TCC Automation grant, or an API-level equivalent); mechanism and grant are stage-1 unresolved questions.

Surfaces divide into two classes:

**Class 1 — native followers (build-time config, zero runtime plumbing):**

- **Ghostty** — native dual-theme: `theme = "light:<light>,dark:<dark>"` with `window-theme = "system"`. Both palette variants are generated at build time from the host's declared scheme pair (`programs.ghostty.themes`), replacing the current single-polarity `stylix.targets.ghostty.enable`. Ghostty repaints open windows itself when the appearance flips (community-config-survey.md §5.1).
...
- **`programs.ghostty.themes` exists at the pinned home-manager rev — green (verified).** Evaluates on neptune's config; its exact submodule shape and interplay with the Stylix target remain implementation questions.
...
- **Still unverified** (carried to Unresolved questions): Ghostty's live repaint of open windows on the appearance flip; the fish/bat end-to-end chain under Ghostty (version floors are necessary, not sufficient — the signal emission and OSC 11 query are untested); the CLI switcher's preference-write mechanism (see Unresolved questions); `desktoppr` behaviour on multi-display; stage-2 Ghostty config-include repointing.
...
- The `programs.ghostty.themes` submodule shape, and whether the Stylix Ghostty target is disabled or coexists (the palette data still derives from Stylix's scheme YAMLs either way — force 2). Note the target also carries font-family and the font-size the `mkForce 13` pin overrides (`home/darwin/ghostty.nix`), so disabling it ripples beyond the palette.
- The CLI switcher's preference-write mechanism: the System Events AppleScript route needs a TCC Automation grant (the same bootstrap class as AeroSpace's Accessibility grant — would join `docs/runbooks/darwin-bootstrap.md`); an API-level alternative may avoid it. Whether the CLI switcher is even part of the stage-1 slice — the native toggle alone exercises the whole chain — is itself an open sequencing choice.
