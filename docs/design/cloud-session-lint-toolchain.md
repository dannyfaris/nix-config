...
| Tool       | Pinned version (nixpkgs `148bab9c1c3c53136ecb44a6ea356a0ed5b39b06`) | Acquisition                                                                                                                                | Integrity pin                                                                                                                                   |
| ---------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
...
| deadnix    | 1.3.2                                                               | `cargo install --git https://github.com/astro/deadnix --rev ba988ddf46f1e57abe3dfacbfbd17fec5a7276d9 --locked --root "$STAGE"`             | git rev (content-addressed), tag `v1.3.2` at pin time; transitive deps by the `Cargo.lock` committed at that rev                                |
...
- **Pin-drift warning (the sync story, see below):** the script reads `flake.lock`'s `nixpkgs` rev with `jq` and prints a one-line warning when it differs from the rev the table was aligned against (`148bab9c1c3c53136ecb44a6ea356a0ed5b39b06`). A warning, not a gate.
...
- **It is a second, hand-maintained version table** in a repo whose lint scope is single-sourced through `statix.toml` → `lib/auto-gen-paths.nix`. Six versions, four URLs, four sha256s and two git revs, none derived from `flake.lock`. That is a genuine drift surface and the accepted-skew decision does not eliminate it, it merely bounds and announces it.
- **It downloads and executes third-party binaries** in an environment with root and repo write access. Three of four sha256s are TOFU — pinning what was downloaded on 2026-08-02, not an upstream attestation, because upstream publishes no checksum file. The two git routes additionally clone and run unsandboxed cargo builds (statix's external fork, deadnix upstream; ~40 transitive crates each), which is *provenance-equal* to CI but not *verification-equal*: nix checks a NAR hash end-to-end, cargo checks per-crate checksums and nothing about the resulting binary.
...
3. **deadnix via cargo-quickinstall prebuilt or `cargo install --locked`?** **`cargo install --locked`** — first-party source, cargo-verified checksum, 13 s, and consistent with the statix route. **Amended 2026-08-03 ([#745](https://github.com/dannyfaris/nix-config/issues/745)):** the crates.io half of that answer did not survive the 1.3.2 re-pin — 1.3.2 is tagged upstream but was never published to crates.io (the sparse index tops out at 1.3.1), so the source is now the upstream git repository pinned by rev (`ba988ddf…`, the commit tag `v1.3.2` pointed at when pinned — pin-by-identity per §Prior art, not by the movable tag) and the checksum clause above holds only for the 1.3.1 era; `--locked` and the first-party trust chain are unchanged, and the route is now literally the statix one.
