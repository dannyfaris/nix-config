...
- **Keep the 2 app-launch handlers** (`Hyper+Return`, `Hyper+B`) in Hammerspoon — but **drop their §7 fullscreen-on-spawn coupling** first. Both currently spawn via `fullscreenAndFocus` (`setFullScreen(true)`), which would throw the new window onto its own native Space instead of letting AeroSpace tile it (the #238/#1615 conflict). Swapping that callback for focus-only makes spawned windows tile; the spawn machinery and the Chrome focus-or-spawn logic are otherwise AeroSpace-fine. Migrating them to `exec-and-forget` (and retiring Hammerspoon from the keybind layer) is a later follow-on the trial can settle.
...
- **Focusing an off-workspace window.** `chromeFocusOrNew`'s focus-existing path may target a window AeroSpace has parked off-screen on another workspace; verify Hammerspoon's `win:focus()` surfaces it and AeroSpace *follows* (switches to that workspace) rather than the two tussling.
