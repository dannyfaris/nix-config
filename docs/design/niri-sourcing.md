...
**Expected to resolve at implementation:** whether `home/nixos/niri-laptop.nix`'s merge survives unchanged under node authoring; how much of `polkit-agent.nix`'s rationale is retired with niri-flake's KDE agent. **Closed (operator-approved, 2026-08-12):** nothing evaluated `lib/niri-config.nix`, so an eval error in it would have gone unnoticed between the slices. `parts/checks.nix` now forces both host shapes to render (`niri-config-renders`, ADR-033's eval-check rung); confirmed to fail closed against an injected regression, not merely to pass today.
