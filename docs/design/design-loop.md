...
2. **Size** — blast-radius / reversibility triage; heavy design for irreversible/cross-cutting, just-build for local/reversible. An option may be eliminated on a stance but not on an *estimated* cost — if cost is the ground, it is measured first. *(Fairbanks risk-driven + Bezos two-way-doors → enforced by the size gate + the skill's stage-2 instruction.)*
...
4. **De-risk** — test the load-bearing assumption before building, and apply the unverified-claims inventory symmetrically: a claim about the outside world is load-bearing whenever it *eliminates an option*, and those are the claims verification skips (§De-risk evidence, #763). *(Fairbanks step 1 → enforced by a required De-risk-evidence section for heavy items, the optional Premises slot, and the checklist's premise rung.)*
...
**The first fully-encoded run — [niri sourcing](./niri-sourcing.md), #763 (2026-08-12).** The test the paragraph below was waiting for: the first run with the whole spine in place (template + `/design` skill + structure lint + dialogue contract). The design was wrong anyway. ~41 files were built across two slices and parked; the change that shipped was 11.

Scorecard:

- **Held:** intent-first (the driving issue is a real problem statement with the routes explicitly not pre-decided, and the note leads with Motivation); the dialogue contract (every stage boundary was an operator gate, forks put as numbered questions with stated leans); weigh-alternatives *as a form* — options were named and argued against the stated forces.
- **Broke — de-risk, asymmetrically.** The stage's own encoded instruction ("state what stays unverified rather than implying coverage you do not have") was *performed*, and performed selectively. The note's unverified-claims inventory listed the rejected fork's "output surface and cache" while the same note's Rationale asserted that fork's *health* as bare uncited fact — and the health claim was the one that eliminated the option. The chosen route's mechanics were meanwhile verified down to store paths.
- **Broke — peer review, mis-aimed.** Nineteen review subagents audited the implementation; none audited the note's external claims, though the checklist's existing "numbers are measured or cited" rung already covered them. An enforced rung pointed at the wrong surface reads as coverage while providing none.
- **The false claims:** the driving issue asserted the maintained fork was "unblessed by sodiboo" and that "11 of its last 12 commits are bot lockfile updates". Both were checkably false when written — the fork's announcement was pinned in the upstream repo three days earlier, and 32 of its last 100 commits are human, from four contributors. An outside commenter caught it; no stage did.

**The validated lesson: verification effort follows the work, and eliminated options get none.** A dead route earns no attention, so the claim that killed it is precisely the claim that never gets checked — while every claim on the surviving route is verified exhaustively. This is the original thesis (a stated-but-unenforced step gets skipped) firing on a *sub-step*: the honesty inventory existed and was applied to one half of the decision. Hence the amendments this run produced — the load-bearing trigger is **"a claim eliminates an option"**, and its binding point is the *ruling*, not de-risk, because in this run the option died in an issue comment a day before the note existed.
