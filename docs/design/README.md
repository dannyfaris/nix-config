...
- **[wiki.md](./wiki.md)** — the personal OS's memory layer: `~/wiki`, an LLM-maintained Obsidian vault in the karpathy LLM-wiki shape, background-synced fleet-wide (mercury excluded; since retired, #634) via home-manager's `services.git-sync`, with git carrying vault config for mobile parity and nix stopping at the vault wall (#506).
...
- **[ephemeral-root.md](./ephemeral-root.md)** — enforcing ADR-034's reprovision-from-source premise at every reboot: a systemd stage-1 btrfs archive-rollback (previous root retained N days under `roots-archive/`, never hard-deleted, fail-safe + kill-switch) plus the impermanence module as the mergeable persist-whitelist surface (module-owns-its-state), `/home` persistent; metis proves first (probe-seeded whitelist, physical-console flip), Alcyone inherits at bootstrap. Extends ADR-034; re-cuts #633's probe as permanent drift-detection (#553). *Proposed; enforcement live on metis (2026-07-30), soaking — ADR and Alcyone bootstrap next.*

- **[fleet-key-custody.md](./fleet-key-custody.md)** — the fleet's identity-architecture hub: untangle neptune's triple-role `id_ed25519` (fleet SSH + GitHub push + sops recovery root) into three single-role key classes — host-key-derived machine identities (reproducible-from-source, NixOS secret-holders only), one standalone operator key as the durable edit/recovery root (vault + an offline copy), and re-homed SSH/GitHub roles. Supersedes #526's bench sequence against the moved fleet (mercury/nixos-vm retired, #634 → recipients drop; darwin decrypts nothing → no host identity needed; ADR-042 holds the SSH slot). Imposes an invariant on #557 (seal the disk, don't TPM-bind the host key, or sops derivation breaks — verified from pinned source) and an atomic-rotation invariant for #572/#566; frozen in [ADR-043](../decisions/ADR-043-fleet-key-custody.md). *Accepted; built.*

- **[conditional-calendar-sync.md](./conditional-calendar-sync.md)** — the operator's calendars unified on electra: a Radicale CalDAV hub as the personal calendar's canonical home (iOS over the tailnet), vdirsyncer transport per spoke (Google CalDAV v2; M365 via a probe-gated DavMail-Graph ladder), and a standalone rules-driven projection daemon (Python, one-shot, rules in Nix-generated TOML, SQLite state, property + mutation tested) applying conditional routing and privacy-shaping between staging and canonical collections. Polling-first, zero public surface; iCloud deferred to phase 2. Research in [`../research/calendar-sync-prior-art.md`](../research/calendar-sync-prior-art.md) (#386). *Proposed; not built.*

- **[ci-docs-only-short-circuit.md](./ci-docs-only-short-circuit.md)** — docs-only PRs stop rebuilding every host closure, without ever skipping a job: the `flake-check` matrix and all three required context names always report, only the command changes (a `checks-without-hosts` flake output derived by exclusion, plus a cache-save guard). Retires ADR-025 §77's `paths-ignore` non-goal on measurement (x86_64-linux median 31.8 min), avoiding rather than accepting the required-checks hang it was rejected for; carries the same issue's case-collision guard, which composes only because the cheap path still runs the pre-commit set (#412). *Accepted; built.*
