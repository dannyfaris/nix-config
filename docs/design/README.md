...
- **[fleet-service-isolation.md](./fleet-service-isolation.md)** — how the fleet isolates a hosted application service: a property-driven tier ladder (native process → OCI container → microVM guest → dedicated host) with one rule — default native, promote to the lowest tier a *named property* demands (no clean packaging, breaking-release cadence, untrusted code, hardware coupling). A host-agnostic `services` capability; metis applies it first. Tiers accrete as services justify them — container tier verified buildable now (keeper #386), guest tier a seam gated on #555. Produces the "central homelab ADR" #387 names. *Proposed; not built.*
