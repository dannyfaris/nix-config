...
- **[work-personal-boundary.md](./work-personal-boundary.md)** — the work/personal split promoted from naming convention to a modelled boundary: a threat model (assets, directions, adversaries led by build-time dependency execution + agent misdirection) driving a layered cut — per-identity agent/credential scoping, confined untrusted execution, dedicated hosts reserved as the strong form. Central finding: file permissions give no isolation within one Unix user, so the boundary needs an execution boundary or a credential broker. Extends ADR-020; upstream of #560 (#570). *Proposed; not built.*
