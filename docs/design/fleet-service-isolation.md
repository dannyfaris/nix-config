...
**Status:** Accepted (2026-08-11) — design note (`docs/design/`). Not built. Originated in #387, which framed this as one half of "the central homelab ADR" before #387's 2026-08-03 re-cut to retiring metis from the fleet outright; the rule below is host-agnostic and stands on its own regardless. The other half (placement) is [fleet-service-placement.md](./fleet-service-placement.md). Upstream of #386, #555; adjacent ADR-034, ADR-030. Drafted 2026-07-07; trimmed same day after adversarial self-review (ladder-as-fleet-policy → a rule plus named seams); accepted 2026-08-11, landing standalone from its placement sibling. Expect an ADR on implementation.

## Summary

When the fleet hosts an application service, the isolation it gets is currently an accident of packaging: ntfy is a native NixOS service because a module exists; keeper (#386), if built, would be a container because it has none. This note replaces the accident with a rule — and deliberately nothing bigger: **default to a native NixOS service; promote along the isolation spectrum (native → OCI container → microVM guest → dedicated host) only to the lowest position a named property demands** (no clean packaging, an upstream cadence that fights the weekly bump, untrusted code, neighbour blast-radius). Only the first two positions are *adopted tiers* — native (proven by ntfy) and container (keeper is the worked example that would force it; verified buildable in the pin). Guest and dedicated-host are **named seams**: vocabulary for promotion targets, deliberately unbuilt (guest gated on #555; dedicated-host reached only through the placement axis). The rule lands as a host-agnostic `services` capability any host composes; electra already proves the native tier (ntfy, #688), but the rule is the fleet's, not any single host's. Isolation depth is one axis; *where* a service runs — its host and failure domain — is the separate axis carried by the sibling note [fleet-service-placement.md](./fleet-service-placement.md).

## Motivation

#387 originally spun this question out as "the central homelab ADR" while it was still the metis re-role epic; #387 has since been re-cut (2026-08-03) to retiring metis from the fleet outright, but the question was never metis-specific and outlives that plan: the fleet already runs services (ntfy, now on electra, the unit-failure-notifier) today, alcyone will burst-host builds, and further services (Home Assistant, n8n) are coming — every one of them faces the same question of *how isolated a hosted service is from its host, its neighbours, and the fleet's version cadence*. Today there is no answer, only a reflex: reach for a native module if nixpkgs has one, a container if it doesn't. That reflex ties a service's isolation to its packaging luck rather than to its risk.
...
5. **F5 — State honesty (ADR-034).** A stateful service — keeper's Postgres + `ENCRYPTION_KEY` is the worked example — would be the first fleet data not covered by reprovision-from-source, exactly the case ADR-034's migration trigger names. The tier model must surface this seam, not obscure it.
...
Verified against `main` and the pin, 2026-07-07; the container-tier claim re-verified 2026-08-11:
...
- **The container tier is buildable now, in-idiom** — `nix eval …electra.config.virtualisation.oci-containers.backend` returns `"podman"` (re-verified 2026-08-11, also true on alcyone); the option accepts `"podman" | "docker"`. So the tier a service like keeper would force is a standard NixOS capability, not a bespoke build. The existing rootless *dev* docker (`modules/nixos/docker.nix`) is separate and unaffected.
...
- **ADR-034's trigger would fire on a service shaped like keeper** — its migration trigger names "a local-only database" explicitly; keeper's Postgres + `ENCRYPTION_KEY` (unimplemented, #386) is exactly that shape. The state seam is a requirement the model must be ready for, not one it can defer building for.
...
- **The container tier is real new capability, and building it should wait for the service that needs it.** No service currently forces it: keeper (#386) is the worked example of the shape that would, but it is unimplemented and not queued. So designing the model now is right (bank the reasoning while the decision is cheap), but *building* the container tier — oci-containers, image pinning, an app's TLS/OAuth seam — should not run ahead of a real service. The note designs; it does not build.
...
- **A1 — native-only, never containers.** Purest declarative form, but it would leave a keeper-shaped service unable to land without an ugly native repackage, and HA's cadence would fight the weekly bump every week. Fails F4 and F2 (blocks real services). Reality does not fit it.
...
- **A3 — hypervisor-everything (a small internal cloud).** Maximal isolation and cadence-independence, unifies #555 — but heaviest (F7), risks making its host load-bearing as hypervisor (F6), is a new paradigm (against ADR-027's "extension, not paradigm"), and depends on unbuilt #555. Over-built for a first service. Rejected as the default; retained as tier 3.
- **A4 — the property-driven rule (chosen).** The only option that satisfies F1–F8 together: native default (ethos, cheap), promotion on named properties (F3/F4), lowest-sufficient (F2/F7), guest/host as named seams (F8). It concedes the rule's crispness is load-bearing (Drawbacks) and the guest tier is unbuilt (De-risk) — both honest, neither fatal. Impact of doing nothing: the next service with keeper's shape lands as a one-off container with no precedent, and the ones after it each re-litigate isolation from scratch.
...
- **Capability naming** — a host-agnostic `services` bundle vs a homelab-flavoured one; leans `services`, settles in the draft/impl.
...
- Out of scope, named not decided: **the placement axis itself** — which host and failure domain a service runs in (the accelerator / watcher / products criticality-class arbitration the operator opened); #555's guest-substrate selection; the TLS/ingress seam (needs >1 service); inference (parked, #387); naming (#368).
