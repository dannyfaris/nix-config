...
**Status:** Rejected (2026-07-07) — design note, kept as an auditable considered-and-rejected record (not built, and will not be). #570 closed won't-do. Extends ADR-020; upstream of #560. No ADR — nothing changed.

**Decision (2026-07-07): Rejected — residual risk consciously accepted.** The operator declined the boundary on two grounds: (1) fluent daily movement between work and personal trees makes any crossing-heavy mechanism (Layers 2–3 below: confined execution, separate Unix users, dedicated hosts) a boundary that gets bypassed in practice — force F4, which this note ranked first-class; (2) the standing overhead is judged disproportionate to the risk for a single-operator personal fleet (ADR-032 proportionality — the operator's call to make). The asymmetric-obligation point (F3 — the employer's code and GitLab token are exposed, not only the operator's own credentials) was raised explicitly and consciously accepted. The threat model and its central finding are retained below because they remain true and should not be re-derived by a future session: within one Unix user, file permissions provide no work/personal isolation.

**Residual mitigation available (adoptable incrementally, no ADR, no reopening).** `direnv` + `nix-direnv` are already enabled fleet-wide (`home/shared/direnv.nix`, ADR-003) and activate automatically on `cd` — the one Layer-1 primitive compatible with ground (1), since it requires no manual context switch. A per-identity `.envrc` (e.g. at `~/grey-st/`) scopes tooling to the right identity, which **defangs misdirection** (wrong-identity / wrong-token accidents) outright, and can **blunt cross-identity token theft by a malicious build-time dependency** — but only to the extent credentials move from global on-disk files (`~/.config/gh`, `~/.config/glab-cli`) to per-tree broker-injected env (sops / 1Password), because direnv scopes the *environment*, not filesystem access: a poisoned dependency otherwise just reads the token file off disk regardless of `$ENV`. It does not isolate execution, and does not by itself protect the on-disk SSH key or the keyring. It is therefore a lightweight *partial* mitigation — a delivery mechanism for the credential-broker path (Layer 1), not the rejected execution boundary — that the operator can adopt per-tree if appetite ever shifts.

## Summary

*(Rejected — see the Decision above. The analysis below is retained as the reasoned record behind that rejection, not a live proposal.)*
