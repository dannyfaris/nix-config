# hostContext for NixOS hosts. The typed option schema and the
...
# default for flakePath — the operator's Linux home.
let
  operator = import ../../lib/operator.nix;
in
import ../../lib/mk-host-context.nix {
  defaultFlakePath = "${operator.linuxHome}/${operator.flakeRepoDirname}";
}
