  ...
  options.ephemeralRoot = {
    ...
    device = lib.mkOption {
      ...
      description = ''
        ...
        orders against this device's unit, and an alias nothing queues
        orders nothing (#553).
      '';
    ...
    (lib.mkIf cfg.enable {
      ...
      boot.initrd = {
        ...
        systemd.services.ephemeral-root-rollback = {
          ...
          # initrd-root-device.target is the load-bearing member: always in
          # the boot transaction, and reaching it implies udev has minted the
          # root partition's symlinks. The cfg.device unit alone is vacuous
          # when nothing queues it (#553's first-enforcing-boot finding).
          # Ordering-only, never wants — a pull inverts the fail-safe into an
